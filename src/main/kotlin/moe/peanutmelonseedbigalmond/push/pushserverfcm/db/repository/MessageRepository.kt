...
interface MessageRepository : CrudRepository<MessageBean, Long> {
    ...
    fun findByOwnerAndNotDeleted(owner: Long, pageable: Pageable): List<MessageBean>

    @Query("select m from MessageBean m where m.topicId=?1 and m.owner=?2 and m.deleted=false order by m.pushTime desc")
    fun queryMessagesByTopicIdAndOwnerAndPushTimeDesc(
        topicId: String,
        owner: Long,
        pageable: Pageable
    ): List<MessageBean>
    ...
    fun queryMessagesByTopicIsNullIdAndOwnerAndPushTimeDesc(owner: Long, pageable: Pageable): List<MessageBean>
    ...
    fun countByTopicIdAndOwnerAndDeleted(topicId: String?, owner: Long, deleted: Boolean): Int

    fun countByOwnerAndDeleted(owner: Long, deleted: Boolean): Int
}